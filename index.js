...
const updatedTimstamp = parseInt(timestamp)

console.log(updatedTimstamp)
...
console.log(`3 dog years equals: ${dogYears(3)}`);
...
function hungryDog(weight, age){
  if(age >= 1) {
    ...
    return pupper(age, weight)
  }
}


const pupper = (age, weight) => {
  if(2/12 <= age <= 4/12) {
    return weight*.1;
...
function vowelCounter(parameter) {
  let counter = 0;
  if(parameter.toLowerCase() === 'sky') counter++;
  const scanner = parameter.toLowerCase().split('');

  let i = 0;
  while(scanner.length > i) {
    if(scanner[i] === 'a' || 
       scanner[i] === 'e' || 
       scanner[i] === 'i' || 
       scanner[i] === 'o' || 
